...
public class MusicBandAdditionalFunctionalityService {
    ...
    public FlatResponseDto findOneWithMinId() {
        Flat band = musicBandRepository.findFirstByOrderByIdAsc()
            .orElseThrow(() ->
    ...
    @Transactional(readOnly = true)
    public List<FlatResponseDto> findAllWithEstablishmentDateAfter(LocalDate date) {
        List<Flat> foundMusicBands = musicBandRepository.findByEstablishmentDateAfter(date);
    ...
    @Transactional
    public FlatResponseDto addSingleToBand(Long musicBandId) {
        Flat band = musicBandRepository.findById(musicBandId)
            .orElseThrow(() ->
        ...
        Flat musicBandSaved = musicBandRepository.save(band);
